    ...
    while True:
        ...
        try:
            ...
            if current_time - last_send_time >= 0.5:
                results = model(frame)
                detected = any((result.boxes.cls == 0).any() for result in results)  # 0 是 YOLOv8 的 "person" 类别
                ...
                if detected:
                    ...
                    encoded_image = None  # 如果没有检测到人类，图片数据为空
                ...
                message = {
                    ...
                    'image': encoded_image,  # 图片数据，如果没有检测到人类，则为 None
                ...
                channel.basic_publish(exchange='',
                                      ...
                                      body=str(message))
                print(f" [x] Sent alert from camera {camera_index} at {timestamp}")
