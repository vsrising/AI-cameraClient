...
# 将图片地址存入 MySQL 数据库
def save_to_database(camera_index, timestamp, image_path,dev_seriesid):
    ...
    try:
        ...
        # 插入摄像头告警记录到数据库
        insert_query = "INSERT INTO camera_alerts (camera_index, timestamp, image_path, dev_seriesid) VALUES (%s, %s, %s, %s)"
        cursor.execute(insert_query, (camera_index, timestamp, image_path, dev_seriesid))
        connection.commit()
...
def callback(ch, method, properties, body):
    ...
    # 将图片路径和相关信息存储到数据库
    save_to_database(camera_index, timestamp, image_path,dev_seriesid)
